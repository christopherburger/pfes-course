...
Standard Python is designed for general software development, not math. Structures like lists in Python have their own set of rules that makes using them in mathematical operations difficult.
...
Libraries are **external** toolsets built that we can bring into our Python code. For numerical data, the industry standard is **NumPy** (Numerical Python).

If you are working on a local machine, you likely need to install the library first using **pip install numpy** in your terminal. Cloud environments like Colab usually have it pre-installed.

### Standard Library Functions and the `import` Statement

A **standard library** is a collection of pre-written code that comes with Python.

**Library functions** perform tasks that programmers commonly need like `print`, `input`, `range`.

These functions are usually viewed by programmers as a “black box” in that we are not concerned with how they perform their task, only that they do it correctly and we understand what format the results will be.

**Modules** are files that stores functions of the standard library. They help organize library functions not built into the interpreter.

To call a function stored in a module, you need to write an import statement. This is written before you use anything within the module and by convertion is written at the top of the program.
...
We use `as np` as a standard convention. It acts as a nickname so we do not have to type "numpy" every time we want to use one of its tools.
...
Another more useful definition when it comes to dealing with vectors is a scalar is a **scaling factor**. It is a quantity that stretches or shrinks something else.
